...
struct RgbColor
{
	...
	//-------------------------------------------------------------------------
	// cypress conversion from uint32 to RgbColor

	RgbColor(uint32_t color) :
		R(color >> 8 & 0xff), G(color & 0xff), B(color >> 16 & 0xff)
	{
	};
	...
	static uint32_t ColorUint(uint8_t r, uint8_t g, uint8_t b){ return r + g << 8 + b < 16; };
